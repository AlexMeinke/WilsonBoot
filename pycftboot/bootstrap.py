...
the space of scaling dimensions for various CFT operators.
All operators used in
...
# Every time we explicitly use it, we should consider
# implementing such a line in C++
...
# Importing them would not make sense because they refer
# back to things in this file
exec(open("pycftboot/common.py").read())
#exec(open("pycftboot/compat_juliboots.py").read())
exec(open("pycftboot/blocks1.py").read())
exec(open("pycftboot/blocks2.py").read())
...
class SDP:
    ...
    def extremal_coefficients(self, dimensions, spin_irreps, nullity = 1):
        ...
        while len(good_rows) > 0 and not fail:
            other_rows = []
            current_rows = []
            current_coeffs = []
            new_dimensions = []
